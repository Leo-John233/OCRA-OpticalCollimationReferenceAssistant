...
set "DIST_APP=%DIST_ROOT%\OCRA"
set "BUILD_ROOT=%PROJECT_ROOT%\py_build\build"
...
rem 复制发布所需文件
if exist "%CONFIG_DIR%" (
    xcopy "%CONFIG_DIR%" "%DIST_APP%\config\" /E /I /Y >nul
    if errorlevel 1 (
        echo [错误] 复制配置目录失败
        exit /b 1
    )
)
if exist "%PROJECT_ROOT%\LICENSE" copy /Y "%PROJECT_ROOT%\LICENSE" "%DIST_APP%\LICENSE" >nul
if exist "%PROJECT_ROOT%\README.md" copy /Y "%PROJECT_ROOT%\README.md" "%DIST_APP%\README.md" >nul
if exist "%PROJECT_ROOT%\README_EN.md" copy /Y "%PROJECT_ROOT%\README_EN.md" "%DIST_APP%\README_EN.md" >nul
if exist "%PROJECT_ROOT%\THIRD_PARTY_NOTICES.md" copy /Y "%PROJECT_ROOT%\THIRD_PARTY_NOTICES.md" "%DIST_APP%\THIRD_PARTY_NOTICES.md" >nul

echo ===================================================
echo [成功] 目录版已生成
echo %DIST_APP%
echo 请发布整个 OCRA 目录 不要只复制 OCRA.exe
echo ===================================================
exit /b 0
